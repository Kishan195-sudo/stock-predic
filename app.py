...
def main():
    ...
    if predict_button:
        ...
        with tab3:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=df["Date"], y=df["Close"], mode="lines", name="Close Price", line=dict(color="#0B5394", width=3)))
            if "MA_20" in df.columns and not df["MA_20"].isna().all():
                fig.add_trace(go.Scatter(x=df["Date"], y=df["MA_20"], mode="lines", name="20-Day MA", line=dict(color="#D62828", width=2, dash="dash")))
            ...
            fig.update_layout(title=f"{ticker} Stock Price with Moving Averages", xaxis_title="Date", yaxis_title=f"Price ({currency_symbol})", hovermode="x unified", template="plotly_white")
            st.plotly_chart(fig, use_container_width=True)

            fig_volume = go.Figure()
            fig_volume.add_trace(go.Bar(x=df["Date"], y=df["Volume"], name="Volume", marker_color="rgba(11, 83, 148, 0.6)"))
